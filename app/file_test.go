...
func TestUploadFile(t *testing.T) {
	...
	uploadAndVerifyPath := func(t *testing.T, timestamp time.Time, filename string,
		...
		expectedPath := fmt.Sprintf("%v/%v/%v", expectedPathPrefix, info.Id, filename)
		if info.Path != expectedPath {
			t.Fatalf("Incorrect path, got:\n%q, expected:\n%q", info.Path, expectedPath)
		}
	}
	...
	tests := []struct {
		...
		data               []byte
		f                  func(t *testing.T, timestamp time.Time, filename string, data []byte, expectedPathPrefix string)
	}{
		{
			title:              "GIF",
			timestamp:          time.Date(2007, 2, 4, 1, 2, 3, 4, time.Local),
			filename:           "test.gif",
			expectedPathPrefix: fmt.Sprintf("20070204/teams/%v/channels/%v/users/%v", teamId, channelId, userId),
			data:               randomGIF,
			f:                  uploadAndVerifyPath,
		},
		{
			title:              "JPEG",
			timestamp:          time.Date(2007, 2, 4, 1, 2, 3, 4, time.Local),
		...
		{
			title:              "simple same timestamp again",
			timestamp:          time.Date(2007, 2, 4, 1, 2, 3, 4, time.Local),
			filename:           "test",
			expectedPathPrefix: fmt.Sprintf("20070204/teams/%v/channels/%v/users/%v", teamId, channelId, userId),
			data:               []byte("abcd"),
		...
		{
			title:              "simple different timestamp",
			timestamp:          time.Date(2008, 3, 5, 1, 2, 3, 4, time.Local),
			filename:           "test",
			expectedPathPrefix: fmt.Sprintf("20080305/teams/%v/channels/%v/users/%v", teamId, channelId, userId),
			data:               []byte("abcd"),
			f:                  uploadAndVerifyPath,
		},
		{
			title:              "simple dot dot",
			timestamp:          time.Date(2009, 3, 5, 1, 2, 3, 4, time.Local),
			filename:           "test",
			expectedPathPrefix: fmt.Sprintf("20090305/teams/%v/channels/%v/users/%v", teamId, channelId, userId),
			data:               []byte("abcd"),
			f: func(t *testing.T,
	...
	for _, test := range tests {
		t.Run(test.title, func(t *testing.T) {
			test.f(t, test.timestamp, test.filename, test.data, test.expectedPathPrefix)
		})
	}
}
